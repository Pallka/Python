...
@users_router.get("/find_similar", response_model=list[schemes.SimilarRecord])
def find_similar(record: str = Query(...), db: Session = Depends(get_db), dependencies=Depends(JWTBearer())):
    all_records = db.query(Record).all()
    if not all_records:
        raise HTTPException(status_code=404, detail="No records found")

    similar_records, num_similar_records = find_similar_records(record, all_records, db)
    return {"record": similar_records}
